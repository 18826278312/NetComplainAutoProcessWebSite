...
public class Point {
	...
	public boolean isInPolygon(String coordinates,String lng,String lat){
		double px = Double.valueOf(lng);
		double py = Double.valueOf(lat);
		ArrayList<Double> polygonXA = new ArrayList<Double>();
		ArrayList<Double> polygonYA = new ArrayList<Double>();
		String[] strList = coordinates.split(";");
		for (String str : strList){
        	String[] points = str.split(",");
            double polygonPoint_x=Double.parseDouble(points[0]);  
            double polygonPoint_y=Double.parseDouble(points[1]);
            polygonXA.add(polygonPoint_x);
            polygonYA.add(polygonPoint_y);
        }
